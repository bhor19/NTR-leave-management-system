...
$sql2="SELECT * FROM dtr where EmpId='$EmpId'  && date(DTR_Date)=curdate() order by id desc ";
$sql3="SELECT * FROM dtr where EmpId='$EmpId' && date(DTR_Date)!=curdate() order by id desc LIMIT 5";
$sql4="SELECT * FROM dtr where EmpId='$EmpId' order by id desc LIMIT 1 ";
$sql5="SELECT count(*) FROM dtr where EmpId='$EmpId'";
...
while($row=mysqli_fetch_array($records1))
{
   ...
   $pdf->SetFont('Arial','BU','15');
  $pdf->cell(0,5,"APPLIED DTR:",0,0,'C');
  
  $pdf->Ln();
$pdf->SetFont('Arial','BU','15');

$pdf->cell(20,10,"EmpId",1,0,'C');
$pdf->cell(30,10,"DTR_Date",1,0,'C');
$pdf->cell(30,10,"DTR_From",1,0,'C');
$pdf->cell(30,10,"DTR_To",1,0,'C');
...
while($row=mysqli_fetch_array($records2))
{
	...
	$pdf->cell(20,10,$row['EmpId'],1,0,'C');
    $pdf->cell(30,10,$row['DTR_Date'],1,0,'C');
    $pdf->cell(30,10,$row['DTR_From'],1,0,'C');
    $pdf->cell(30,10,$row['DTR_To'],1,0,'C');
 ...
 $pdf->SetFont('Arial','BU','15');
  $pdf->Ln();
 $pdf->Ln();
  $pdf->cell(58,0,"TOTAL DTR TAKEN:",0,0,'L');
